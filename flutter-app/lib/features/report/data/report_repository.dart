...
final reportRepositoryProvider = Provider<ReportRepository>((ref) {
  ...
  return ReportRepository(reportApiClient, aiApiClient);
});
...
class ReportRepository {
  ...
  ReportRepository(this._reportApiClient, this._aiApiClient);
  ...
  Future<AIAnalysisResult> analyzeImage(String imageId, {ReportType? expectedType}) async {
    try {
      // 이미지 파일을 가져와서 MultipartFile로 변환
      // 실제로는 imageId로 이미지를 가져오는 로직이 필요
      // 임시로 XFile을 사용하는 방법을 제공
