...
class LocationService {
  ...
  Future<Map<String, dynamic>> getLocationServiceStatus() async {
    try {
      ...
      return {'error': e.toString(), 'canGetLocation': false};
    }
  }
  ...
  Future<void> debugLocationService() async {
    try {
      ...
      try {
        ...
        if (lastPosition != null) {
          print(
            '📍 마지막 위치: ${lastPosition.latitude}, ${lastPosition.longitude}',
          );
  ...
  Future<void> checkLocationProviders() async {
    try {
      ...
      try {
        ...
        print(
          '📶 네트워크 위치: ${networkPosition.latitude}, ${networkPosition.longitude}',
        );
