...
class RoboflowService {
    ...
    try {
      ...
      if (!validApiKey) {
        ...
        for (final detection in result.detections) {
          debugPrint('   - ${detection.koreanName}: ${detection.confidencePercent}');
        }
        return result;
      }
      ...
      for (final prediction in predictions) {
        try {
          ...
          if (detection.confidence >= 0.5) {
            ...
            debugPrint('✅ 백엔드 감지: ${detection.className} (${detection.confidencePercent})');
          }
        } catch (e) {
    ...
    try {
      ...
      for (final prediction in predictions) {
        try {
          ...
          if (detection.confidence >= 0.5) {
            detections.add(detection);
            debugPrint('✅ 감지된 객체: ${detection.className} (${detection.confidencePercent})');
          } else {
            debugPrint('⚠️ 낮은 신뢰도로 제외: ${detection.className} (${detection.confidencePercent})');
          }
        } catch (e) {
      ...
      debugPrint('🎯 최종 결과: ${detections.length}개 객체, 평균 신뢰도: ${(avgConfidence * 100).toInt()}%');
  ...
  ObjectDetectionResult _getMockDetectionResult(File imageFile) {
    ...
    for (int i = 0; i < numDetections; i++) {
      ...
      } while (selectedClasses.contains(className) && selectedClasses.length < possibleClasses.length);
  ...
  Future<bool> testConnection() async {
    try {
      ...
      final success = response.statusCode == 200 || response.statusCode == 400; // 400은 파라미터 부족으로 인한 것으로 연결은 성공
      
      debugPrint(success ? '✅ 연결 테스트 성공' : '❌ 연결 테스트 실패: ${response.statusCode}');
      return success;
