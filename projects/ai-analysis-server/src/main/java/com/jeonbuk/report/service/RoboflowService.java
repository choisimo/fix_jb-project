...
import com.jeonbuk.report.domain.repository.ReportCategoryRepository;
...
public class RoboflowService {
    ...
    private Map<String, Object> executeImageAnalysisWithRetry(MultipartFile imageFile, int confidence, int overlap, long startTime) throws Exception {
        ...
        for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
            try {
                ...
                if (response.getStatusCode().is2xxSuccessful()) {
                    ...
                    lastException = new RuntimeException("API 응답 오류: " + response.getStatusCode());
                    log.warn("API 응답 오류 (시도 {}/{}): 상태 코드 {}", attempt, MAX_RETRY_ATTEMPTS, response.getStatusCode());
                }
            } catch (Exception e) {
                lastException = e;
        ...
        if (lastException != null) {
            log.error("Roboflow API 호출 최종 실패. 마지막 오류: {}", lastException.getMessage());
            throw new RuntimeException("Roboflow API 분석에 최종적으로 실패했습니다", lastException);
        } else {
            // 이 경우는 이론적으로 발생하기 어렵지만, 안전을 위해 처리
            log.error("Roboflow API 호출 최종 실패. 알 수 없는 오류.");
            throw new RuntimeException("Roboflow API 분석에 최종적으로 실패했습니다. (알 수 없는 오류)");
        }
    }
    ...
    private Map<String, Object> createMockAnalysisResponse(MultipartFile imageFile, int confidence, int overlap, long startTime) {
        ...
        log.info("🎭 모의 AI 분석 완료 - 파일: {}, 감지된 객체: {}개", filename, predictions.size());
    ...
    private void validateConfiguration() {
        if (apiKey == null || apiKey.isEmpty()) {
            log.error("❌ Roboflow API key가 설정되지 않았습니다. 서비스가 비활성화됩니다.");
