...
- **작성일**: 2025년 7월 11일
- **프로젝트 타입**: Flutter 모바일/웹 + Spring Boot 백엔드 + AI 분석 서비스
- **개발 방식**: 크로스플랫폼 (Flutter) + 마이크로서비스 (Spring Boot) + AI 분석 파이프라인
- **기술 스택**: Spring Boot, Spring Security, JPA, QueryDSL, **PostgreSQL (PostGIS)**, **Redis**, Kafka, Docker, FastAPI, Roboflow, Google ML Kit
- **인증 방식**: OAuth2 + JWT
- **배포 방식**: Docker Compose + Nginx Load Balancing + CI/CD
- **테스트/운영 환경**: CBT(Closed Beta Test) + 자동화 테스트 + 실시간 모니터링
- **주요 변경사항**: AI 분석 기능, 데이터베이스 구조 확장, Flutter 앱 기능 강화, 백엔드 서비스 통합, 다양한 테스트/운영 스크립트 추가
- **문서 최신화일**: 2025년 7월 11일
...
graph TB
    ...
    subgraph "Data & Message Layer"
        I[📄 PostgreSQL (PostGIS)<br/>Primary Database]
...
### 3. 테스트 로그인
- 앱 실행 후 로그인 화면에서 테스트 계정 입력
- 또는 하단의 테스트 계정 버튼 클릭 시 자동 입력
...
### 🎯 핵심 AI 기능
- Roboflow 객체 인식 + Google ML Kit OCR을 활용한 이미지/텍스트 자동 분류 및 설명 생성
- AI 분석 결과는 `analyze_image.py` 및 `roboflow_test.py` 참고

#### 1. 📸 스마트 이미지 분석
- 16개 클래스 감지: 도로 파손, 포트홀, 무단 투기, 낙서, 간판 파손 등
- 실시간 분석(평균 250ms 내 처리)
- 신뢰도 85% 이상
- 자동 카테고리/우선순위 추천

#### 2. 📝 텍스트 인식 (Google ML Kit OCR)
- 한글 특화: 간판, 표지판, 주소 정보 인식
- 정보 추출: 주소, 전화번호, 업체명 자동 파싱

#### 3. 🧑‍💻 사용자 경험 혁신
- ⚡ 3분 내 신고서 완성
- 오프라인 지원, 빠른 동기화, 로컬 AI 처리
...
| **공사/안전**   | 공사 문제, 전기 위험                | 공사 안내문, 업체 정보 |
...
#### 6. 성능 최적화
- 이미지 압축, 동시 분석, 결과 로컬 저장 등

#### 7. AI 기능 사용 팁
- 충분한 조명 확보, 적절한 거리 유지(2-5m), 대상 객체 중앙 배치, 텍스트는 정면 촬영 권장
- AI 분석 오류 시 수동 카테고리/우선순위 조정 가능
...
- [SPRING_BOOT_IMPLEMENTATION_GUIDE.md](../spring-backend/SPRING_BOOT_IMPLEMENTATION_GUIDE.md)
