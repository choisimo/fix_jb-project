...
AI_HEALTH=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8081/api/v1/ai/health 2>/dev/null)
...
echo "📊 AI 기능 목록 확인..."
AI_CAPS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8081/api/v1/ai/capabilities 2>/dev/null)
if [ "$AI_CAPS" = "200" ]; then
    echo "✅ AI 기능 목록: 정상"
    curl -s http://localhost:8081/api/v1/ai/capabilities | jq '.capabilities[]' 2>/dev/null || echo "응답 받음"
else
...
if [ -f "test_images/pothole_1.jpg" ]; then
    ...
    ANALYSIS_RESULT=$(curl -s -o /dev/null -w "%{http_code}" \
        ...
        http://localhost:8081/api/v1/ai/analyze/comprehensive 2>/dev/null)
...
echo ""
echo "🎭 Mock 데이터 사용 확인..."
MOCK_COUNT=$(docker logs jbreport-ai-analysis --tail 50 2>/dev/null | grep -c "모의\|mock\|Mock" || echo "0")
